...
from SmartTraderFrameWork.trader.stGateway import Gateway
from SmartTraderFrameWork.api.ctp.vnctpmd import MdApi
from SmartTraderFrameWork.api.ctp.vnctptd import TdApi
from SmartTraderFrameWork.api.ctp.ctp_data_type import defineDict
from SmartTraderFrameWork.trader.language import text
from SmartTraderFrameWork.trader.language import constant
from SmartTraderFrameWork.trader.stFunction import  getJsonPath, getTempPath
# ctpGateway包中的常量
# from .language import text
from SmartTraderFrameWork.trader.gateway.ctpGateway.language import text
from SmartTraderFrameWork.trader.stModel import *

# 以下为一些ST类型和CTP类型的映射字典
# 价格类型映射
priceTypeMap = {}
priceTypeMap[constant.PRICETYPE_LIMITPRICE] = defineDict["THOST_FTDC_OPT_LimitPrice"]
priceTypeMap[constant.PRICETYPE_MARKETPRICE] = defineDict["THOST_FTDC_OPT_AnyPrice"]
priceTypeMapReverse = {v: k for k, v in priceTypeMap.items()}

# 方向类型映射
directionMap = {}
directionMap[constant.DIRECTION_LONG] = defineDict['THOST_FTDC_D_Buy']
directionMap[constant.DIRECTION_SHORT] = defineDict['THOST_FTDC_D_Sell']
directionMapReverse = {v: k for k, v in directionMap.items()}

# 开平类型映射
offsetMap = {}
offsetMap[constant.OFFSET_OPEN] = defineDict['THOST_FTDC_OF_Open']
offsetMap[constant.OFFSET_CLOSE] = defineDict['THOST_FTDC_OF_Close']
offsetMap[constant.OFFSET_CLOSETODAY] = defineDict['THOST_FTDC_OF_CloseToday']
offsetMap[constant.OFFSET_CLOSEYESTERDAY] = defineDict['THOST_FTDC_OF_CloseYesterday']
offsetMapReverse = {v:k for k,v in offsetMap.items()}

# 交易所类型映射
exchangeMap = {}
exchangeMap[constant.EXCHANGE_CFFEX] = 'CFFEX'
exchangeMap[constant.EXCHANGE_SHFE] = 'SHFE'
exchangeMap[constant.EXCHANGE_CZCE] = 'CZCE'
exchangeMap[constant.EXCHANGE_DCE] = 'DCE'
exchangeMap[constant.EXCHANGE_SSE] = 'SSE'
exchangeMap[constant.EXCHANGE_SZSE] = 'SZSE'
exchangeMap[constant.EXCHANGE_INE] = 'INE'
exchangeMap[constant.EXCHANGE_UNKNOWN] = ''
exchangeMapReverse = {v:k for k,v in exchangeMap.items()}

# 持仓类型映射
posiDirectionMap = {}
posiDirectionMap[constant.DIRECTION_NET] = defineDict["THOST_FTDC_PD_Net"]
posiDirectionMap[constant.DIRECTION_LONG] = defineDict["THOST_FTDC_PD_Long"]
posiDirectionMap[constant.DIRECTION_SHORT] = defineDict["THOST_FTDC_PD_Short"]
posiDirectionMapReverse = {v:k for k,v in posiDirectionMap.items()}

# 产品类型映射
productClassMap = {}
productClassMap[constant.PRODUCT_FUTURES] = defineDict["THOST_FTDC_PC_Futures"]
productClassMap[constant.PRODUCT_OPTION] = defineDict["THOST_FTDC_PC_Options"]
productClassMap[constant.PRODUCT_COMBINATION] = defineDict["THOST_FTDC_PC_Combination"]
productClassMapReverse = {v:k for k,v in productClassMap.items()}
productClassMapReverse[defineDict["THOST_FTDC_PC_ETFOption"]] = constant.PRODUCT_OPTION
productClassMapReverse[defineDict["THOST_FTDC_PC_Stock"]] = constant.PRODUCT_EQUITY

# 委托状态映射
statusMap = {}
statusMap[constant.STATUS_ALLTRADED] = defineDict["THOST_FTDC_OST_AllTraded"]
statusMap[constant.STATUS_PARTTRADED] = defineDict["THOST_FTDC_OST_PartTradedQueueing"]
statusMap[constant.STATUS_NOTTRADED] = defineDict["THOST_FTDC_OST_NoTradeQueueing"]
statusMap[constant.STATUS_CANCELLED] = defineDict["THOST_FTDC_OST_Canceled"]
...
class  CtpGateway(Gateway):
    '''ctp网关'''
    #----------------------------------------------------------------------
    def __init__(self, eventEngine=None, gatewayName='CTP',logEngine=None):
    ...
    def connect(self):
        try:
            ...
            # LogData数量类型，由stGateway引用stModel包，由于继承而被引入本模块
            log = LogData() # 日志数据类
        ...
        try:
            ...
            log = LogData()  # 日志数据类
            log.gatewayName = self.gatewayName
            log.logContent = 'userID = %s,  password = %s, brokerID = %s, tdAddress = %s, mdAddress = %s'%(userID,
                                                                                                       password,
                                                                                                       brokerID,
                                                                                                       tdAddress,
                                                                                                       mdAddress)
            ...
            if 'authCode' in setting:
                ...
                # for python3 的接口中没有requireAuthentication属性
...
class CtpMdApi(MdApi):
    ...
    def __init__(self, gateway):
        ...
        self.reqID = constant.EMPTY_INT  # 操作请求编号
        ...
        self.userID = constant.EMPTY_STRING  # 账号
        self.password = constant.EMPTY_STRING  # 密码
        self.brokerID = constant.EMPTY_STRING  # 经纪商代码
        self.address = constant.EMPTY_STRING  # 服务器地址
    ...
    def onRtnDepthMarketData(self, data):
        ...
        tick.stSymbol = tick.symbol  # '.'.join([tick.symbol, tick.exchange])
        ...
        # 大商所日期转换
        if tick.exchange is constant.EXCHANGE_DCE:
...
class CtpTdApi(TdApi):
    ...
    def __init__(self, gateway):
        ...
        self.reqID = constant.EMPTY_INT  # 操作请求编号
        self.orderRef = constant.EMPTY_INT  # 订单编号
        ...
        self.frontID = constant.EMPTY_INT  # 前置机编号
        self.sessionID = constant.EMPTY_INT  # 会话编号
    ...
    def onRspOrderInsert(self, data, error, n, last):
        ...
        order.stSymbol = order.symbol
        order.orderID = data['OrderRef']
        order.stOrderID = '.'.join([self.gatewayName, order.orderID])
    ...
    def onRspQryInvestorPosition(self, data, error, n, last):
        ...
        if posName in self.posDict:
            ...
            pos.stSymbol = pos.symbol
            pos.direction = posiDirectionMapReverse.get(data['PosiDirection'], '')
            pos.stPositionName = '.'.join([pos.stSymbol, pos.direction])
    ...
    def onRspQryTradingAccount(self, data, error, n, last):
        ...
        account.stAccountID = '.'.join([self.gatewayName, account.accountID])
    ...
    def onRspQryInstrument(self, data, error, n, last):
        ...
        contract.stSymbol = contract.symbol  # '.'.join([contract.symbol, contract.exchange])
    ...
    def onRtnOrder(self, data):
        ...
        order.symbol = data['InstrumentID']
        order.exchange = exchangeMapReverse[data['ExchangeID']]
        order.stSymbol = order.symbol  # '.'.join([order.symbol, order.exchange])

        order.orderID = data['OrderRef']
        # CTP的报单号一致性维护需要基于frontID, sessionID, orderID三个字段
        # 但在本接口设计中，已经考虑了CTP的OrderRef的自增性，避免重复
        # 唯一可能出现OrderRef重复的情况是多处登录并在非常接近的时间内（几乎同时发单）
        # 考虑到StTrader的应用场景，认为以上情况不会构成问题
        order.stOrderID = '.'.join([self.gatewayName, order.orderID])
    ...
    def onRtnTrade(self, data):
        ...
        trade.stSymbol = trade.symbol  # '.'.join([trade.symbol, trade.exchange])

        trade.tradeID = data['TradeID']
        trade.stTradeID = '.'.join([self.gatewayName, trade.tradeID])

        trade.orderID = data['OrderRef']
        trade.stOrderID = '.'.join([self.gatewayName, trade.orderID])
    ...
    def sendOrder(self, orderReq):
        ...
        stOrderID = '.'.join([self.gatewayName, str(self.orderRef)])
        return stOrderID
